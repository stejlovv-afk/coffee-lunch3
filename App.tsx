...
const App: React.FC = () => {
  ...
  // --- Reviews Logic ---
  const addReview = (productId: string, text: string, rating: number) => {
    const newReview: Review = {
      id: Date.now().toString(),
      productId,
      author: 'Посетитель', // In real app, get from Telegram user
      text,
      rating,
      ...
      <header className="sticky top-0 z-20 bg-white/80 backdrop-blur-md shadow-sm px-4 py-3 flex justify-between items-center">
        ...
        <button 
          onClick={() => {
            ...
            // Simple filter mode for demo
            alert("Фильтр по избранному в разработке (UI Demo)"); 
          }}
          className="p-2 bg-coffee-50 rounded-full text-coffee-500"
        >
          ...
          >
            <div className="relative mb-2">
              ...
              <button 
                ...
                className="absolute top-2 right-2 p-1.5 bg-white/60 backdrop-blur rounded-full text-red-500"
              >
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 p-4 z-40 bg-gradient-to-t from-white via-white/80 to-transparent pointer-events-none">
          <button 
            ...
            className="w-full pointer-events-auto bg-coffee-500/90 backdrop-blur-md border border-white/20 text-white rounded-3xl p-4 shadow-xl flex items-center justify-between active:scale-[0.98] transition-all"
          >
