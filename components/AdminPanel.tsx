...
const AdminPanel: React.FC<AdminPanelProps> = ({ 
  ...
  const [newVariants, setNewVariants] = useState<{size: string, price: string}[]>([{ size: 'порция', price: '' }]);
  ...
  const filteredMenuItems = products.filter(item => item.name.toLowerCase().includes(searchTerm.toLowerCase()));
  const filteredEditorItems = products.filter(item => item.name.toLowerCase().includes(editorSearchTerm.toLowerCase()));
  ...
  const startEdit = (product: Product) => {
      ...
      setNewVariants(product.variants && product.variants.length > 0 
          ...
          : [{ size: 'порция', price: String(product.price || 0) }]);
  ...
  const startAdd = () => {
      ...
      setNewCat('coffee');
      setNewImage('');
      setNewVariants([{ size: 'порция', price: '' }]);
      setModifiers({});
      setEditorMode('form');
      setActiveTab('editor');
  };

  const addVariant = () => setNewVariants([...newVariants, { size: '', price: '' }]);
  const removeVariant = (index: number) => setNewVariants(newVariants.filter((_, i) => i !== index));
  ...
  const handleShiftClick = () => !isShiftClosed ? setShowShiftConfirm(true) : onToggleShift(false);
  const confirmCloseShift = () => { onToggleShift(true); setShowShiftConfirm(false); };

  const handleEditorSubmit = () => {
      if (!newName || !newImage || newVariants.some(v => !v.price)) {
          alert("Заполните все поля названия, фото и цены у всех объемов.");
          return;
      }
      ...
      const payload = {
          name: newName,
          category: newCat,
          variants: newVariants.map(v => ({ size: v.size || 'порция', price: Number(v.price) })),
          image: newImage,
          modifiers: modifiers
      };
  ...
  const handleDelete = (id: string) => {
      if (confirm("Удалить этот товар?")) { onDeleteProduct([id]); setEditorMode('list'); }
  };
  ...
  const toggleSelectId = (id: string) => {
      setSelectedDeleteIds(prev => prev.includes(id) ? prev.filter(i => i !== id) : [...prev, id]);
  };
  ...
  const updateStock = (id: string, value: string) => {
      ...
      if (value === '') delete newInventory[id];
      else newInventory[id] = parseInt(value, 10);
    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xl flex flex-col animate-slide-up">
      <div className="p-4 border-b border-white/10 glass-panel">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={onClose} disabled={isLoading} className="text-brand-muted font-medium hover:text-white transition-colors">{isLoading ? '' : 'Закрыть'}</button>
        </div>

        <div className="mb-4">
             <button onClick={handleShiftClick} disabled={isLoading} className={`w-full py-3 rounded-xl font-bold text-sm shadow-md transition-all flex items-center justify-center gap-2 ${isShiftClosed ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
      ...
      {activeTab === 'menu' && (
        <>
            <div className="px-4 pb-2"><div className="relative"><SearchIcon className="absolute left-3 top-3 w-5 h-5 text-brand-muted" /><input type="text" placeholder="Найти товар..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full glass-input rounded-xl py-2.5 pl-10 pr-4 text-white placeholder:text-brand-muted/50 focus:outline-none focus:border-brand-yellow/50" /></div></div>
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
                ...
                return (
                    ...
                    <div className="flex items-center gap-3"><img src={item.image} className={`w-10 h-10 rounded-full object-cover border border-white/10 ${isHidden ? 'opacity-50 grayscale' : ''}`} /><div className="flex flex-col"><span className={`font-bold text-sm ${isHidden ? 'text-red-400 line-through' : 'text-brand-text'}`}>{item.name}</span><span className="text-[10px] text-brand-muted">{item.variants[0]?.price || 0}₽</span></div></div>
                    <div className="flex items-center gap-2">
                         <div className="flex flex-col items-end gap-1" onClick={(e) => e.stopPropagation()}><span className="text-[10px] font-bold text-brand-muted uppercase">Наличие</span><input type="number" placeholder="∞" className="w-12 py-1 px-1 text-center bg-black/40 border border-white/10 rounded-lg text-xs text-white focus:border-brand-yellow outline-none" value={stock !== undefined ? stock : ''} onChange={(e) => updateStock(item.id, e.target.value)} /></div>
              ...
              {editorMode === 'list' ? (
                  <div className="space-y-4">
                      <div className="flex gap-2 mb-2">
                          <button onClick={startAdd} className="flex-1 py-3 bg-brand-yellow text-black rounded-xl font-bold flex items-center justify-center gap-2"><PlusIcon className="w-5 h-5" /> Добавить</button>
                          {selectedDeleteIds.length > 0 && <button onClick={handleBulkDelete} className="flex-1 py-3 bg-red-500 text-white rounded-xl font-bold flex items-center justify-center gap-2 animate-fade-in"><TrashIcon className="w-5 h-5" /> Удалить ({selectedDeleteIds.length})</button>}
                      </div>
                      <div className="relative mb-4"><SearchIcon className="absolute left-3 top-3 w-5 h-5 text-brand-muted" /><input type="text" placeholder="Поиск для редактирования..." value={editorSearchTerm} onChange={(e) => setEditorSearchTerm(e.target.value)} className="w-full glass-input rounded-xl py-2.5 pl-10 pr-4 text-white placeholder:text-brand-muted/50 focus:outline-none focus:border-brand-yellow/50" /></div>
                              ...
                              {filteredEditorItems.map(item => (
                                  <div key={item.id} className={`flex items-center justify-between p-3 rounded-xl border transition-colors ${selectedDeleteIds.includes(item.id) ? 'bg-red-500/10 border-red-500/30' : 'bg-white/5 border-white/5'}`}>
                                      <div className="flex items-center gap-3 overflow-hidden flex-1">
                                          <div onClick={() => toggleSelectId(item.id)} className={`w-5 h-5 rounded border flex items-center justify-center cursor-pointer transition-colors ${selectedDeleteIds.includes(item.id) ? 'bg-red-500 border-red-500' : 'border-white/20'}`}>{selectedDeleteIds.includes(item.id) && <div className="w-2.5 h-2.5 bg-white rounded-sm" />}</div>
                                          <img src={item.image} className="w-10 h-10 rounded-full object-cover flex-shrink-0" onClick={() => startEdit(item)} />
                                          <div className="min-w-0" onClick={() => startEdit(item)}><h4 className="font-bold text-sm text-white truncate">{item.name}</h4><p className="text-xs text-brand-muted">{item.variants[0]?.price || 0}₽</p></div>
                                      </div>
                                      <div className="flex gap-2 flex-shrink-0"><button onClick={() => startEdit(item)} className="p-2 bg-blue-500/10 text-blue-400 rounded-lg border border-blue-500/20">✏️</button></div>
                                  </div>
                              ))}
                          </div>
                      )}
                  ...
                  <div className="space-y-4 pb-10">
                      <div className="flex items-center justify-between mb-2"><h3 className="text-white font-bold text-lg">{editId ? 'Редактирование' : 'Новый товар'}</h3><button onClick={() => setEditorMode('list')} className="text-sm text-brand-muted">Назад</button></div>
                      ...
                      <div className="bg-white/5 p-4 rounded-xl border border-white/10 space-y-3">
                          ...
                          {newVariants.map((v, i) => (
                              <div key={i} className="flex gap-2 items-center">
                                  <input type="text" value={v.size} onChange={e => updateVariant(i, 'size', e.target.value)} placeholder="Объем (0.2л...)" className="flex-1 glass-input p-2 rounded-lg text-white outline-none text-sm" />
                                  <input type="number" value={v.price} onChange={e => updateVariant(i, 'price', e.target.value)} placeholder="Цена (₽)" className="w-24 glass-input p-2 rounded-lg text-white outline-none text-sm" />
                                  {newVariants.length > 1 && <button onClick={() => removeVariant(i)} className="text-red-400 p-2 hover:bg-red-500/20 rounded-lg"><TrashIcon className="w-4 h-4" /></button>}
                              </div>
                          ))}
                          <button onClick={addVariant} className="w-full py-2 border border-dashed border-white/20 rounded-lg text-brand-muted text-sm hover:text-white transition-colors">+ Добавить объем</button>
                      </div>

                      <div><label className="text-xs text-brand-muted uppercase font-bold ml-1">Категория</label><div className="grid grid-cols-2 gap-2 mt-1">{CATEGORIES.map(cat => (<button key={cat.id} onClick={() => setNewCat(cat.id)} className={`py-2 rounded-lg text-xs font-bold border ${newCat === cat.id ? 'bg-brand-yellow text-black border-brand-yellow' : 'bg-white/5 text-brand-muted border-white/10'}`}>{cat.label}</button>))}</div></div>
                      ...
                      <div className="bg-white/5 p-4 rounded-xl border border-white/10 space-y-3">
                          ...
                          <div className="flex items-center justify-between"><span className="text-sm text-white">Молоко</span><input type="checkbox" checked={modifiers.hasMilk || false} onChange={e => toggleModifier('hasMilk', e.target.checked)} className="accent-brand-yellow w-5 h-5" /></div>
                          <div className="flex items-center justify-between"><span className="text-sm text-white">Сироп</span><input type="checkbox" checked={modifiers.hasSyrup || false} onChange={e => toggleModifier('hasSyrup', e.target.checked)} className="accent-brand-yellow w-5 h-5" /></div>
                          <div className="flex items-center justify-between"><span className="text-sm text-white">Сахар</span><input type="checkbox" checked={modifiers.hasSugar || false} onChange={e => toggleModifier('hasSugar', e.target.checked)} className="accent-brand-yellow w-5 h-5" /></div>
                          <div className="flex items-center justify-between"><span className="text-sm text-white">Корица</span><input type="checkbox" checked={modifiers.hasCinnamon || false} onChange={e => toggleModifier('hasCinnamon', e.target.checked)} className="accent-brand-yellow w-5 h-5" /></div>
                          <div className="flex items-center justify-between"><span className="text-sm text-white">Можно Соус</span><input type="checkbox" checked={modifiers.hasSauce || false} onChange={e => toggleModifier('hasSauce', e.target.checked)} className="accent-brand-yellow w-5 h-5" /></div>
                          <div className="flex items-center justify-between"><span className="text-sm text-white">Можно Температуру</span><input type="checkbox" checked={modifiers.hasTemp || false} onChange={e => toggleModifier('hasTemp', e.target.checked)} className="accent-brand-yellow w-5 h-5" /></div>
                          <div className="flex items-center justify-between"><span className="text-sm text-white">Нужны приборы</span><input type="checkbox" checked={modifiers.needsCutlery || false} onChange={e => toggleModifier('needsCutlery', e.target.checked)} className="accent-brand-yellow w-5 h-5" /></div>
                          <div className="flex items-center justify-between"><span className="text-sm text-white">Хот-Дог Опции</span><input type="checkbox" checked={modifiers.isHotDog || false} onChange={e => toggleModifier('isHotDog', e.target.checked)} className="accent-brand-yellow w-5 h-5" /></div>
                          <div>
                      ...
                      <div className="space-y-2">
                          {promoCodes.map(p => (
                             <div key={p.code} className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
                                 <div><h4 className="font-bold text-brand-yellow tracking-wider">{p.code}</h4><div className="text-[10px] text-brand-muted flex gap-2"><span>-{p.discountPercent}%</span>{p.firstOrderOnly && <span className="text-blue-400">• Только первый</span>}</div></div>
      ...
      {activeTab === 'revenue' && (
          <div className="flex-1 p-4 space-y-4 overflow-y-auto">
              <div className="glass-panel p-6 rounded-2xl border border-brand-yellow/20 relative overflow-hidden group"><h3 className="text-brand-muted text-sm font-bold uppercase tracking-widest mb-1">Выручка за сегодня</h3><div className="text-4xl font-black text-white drop-shadow-md">{dailyRevenue.toLocaleString('ru-RU')} <span className="text-brand-yellow">₽</span></div></div>
               <div className="glass-panel p-6 rounded-2xl border border-purple-500/20 relative overflow-hidden group"><h3 className="text-brand-muted text-sm font-bold uppercase tracking-widest mb-1">Выручка за месяц</h3><div className="text-4xl font-black text-white drop-shadow-md">{monthlyRevenue.toLocaleString('ru-RU')} <span className="text-purple-400">₽</span></div></div>
          </div>
      )}
