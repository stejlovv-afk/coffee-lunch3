    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xl flex flex-col animate-slide-up">
      ...
      <div className="p-4 border-b border-white/10 glass-panel">
        ...
        <div className="flex bg-black/40 p-1 rounded-xl mb-4 overflow-x-auto no-scrollbar gap-1">
            ...
            <button onClick={() => setActiveTab('add')} className={`flex-1 min-w-[60px] py-2 rounded-lg text-[10px] font-bold transition-all ${activeTab === 'add' ? 'bg-white/10 text-brand-yellow shadow-md' : 'text-brand-muted'}`}>Добавить</button>
      ...
      {activeTab === 'add' && (
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
              ...
              <div className="pt-4"><button onClick={handleAddSubmit} disabled={isLoading} className="w-full py-3 bg-white/10 hover:bg-white/20 text-white rounded-xl font-bold border border-white/20 flex items-center justify-center gap-2"><PlusIcon className="w-5 h-5" /> Добавить товар</button></div>
          </div>
      )}
              ...
              {customItems.length === 0 ? (<div className="text-center text-brand-muted py-10 opacity-50">Нет добавленных товаров</div>) : (
                  customItems.map((item) => (
                    <div key={item.id} className="flex items-center justify-between p-3 rounded-xl border border-white/5 bg-white/5">
                        <div className="flex items-center gap-3"><img src={item.image} className="w-10 h-10 rounded-full object-cover border border-white/10" /><div><h4 className="font-bold text-sm text-brand-text">{item.name}</h4><p className="text-[10px] text-brand-muted">{item.variants[0].price}₽</p></div></div>
